...
exports.cancelFriendRequest = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const recipientRequestRef = toUserRef.collection('friendRequests').doc(fromUserId);
        ...
        console.log(`Friend request from ${fromUserId} to ${toUserId} has been canceled.`);
        return { success: true, message: 'Friend request canceled successfully.' };

    } catch (error) {
        console.error('Error canceling friend request:', error);
        throw new functions.https.HttpsError('internal', 'Unable to cancel friend request.');
    }
});
